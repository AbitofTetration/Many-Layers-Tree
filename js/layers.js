...
addLayer("e", {
    ...
    startData() { return {
        ...
        clickables: {[11]: new ExpantaNum(0)}, // Optional default Clickable state
    }},
    color: "#ffc940",
        ...
        buyables: {
            11: {
                ...
                effect(x=player[this.layer].buyables[this.id]) { // Effects of owning x of the items, x is a ExpantaNum
                    ...
                    if (getClickableState(this.layer, 11).gte(1)) eff = eff.times(tmp[this.layer].clickables[11].effect)
                    return eff;
                }, 
                display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
    ...
    clickables: {
        11: {
            ...
            display() { // Everything else displayed in the buyable button after the title
                ...
                Requires "+format(tmp[this.layer].clickables[this.id].goal)+" fourth matter dimensions."
            },
            goal() {
                let data = getClickableState(this.layer, this.id)
                return ExpantaNum.mul(3, data.add(1))
            },
            effect() {
              let data = getClickableState(this.layer, this.id)
            ...
            canClick() {
                return player[this.layer].buyables[14].gte(tmp[this.layer].clickables[this.id].goal) },
            onClick() { 
                if (!player[this.layer].buyables[14].gte(tmp[this.layer].clickables[this.id].goal)) {
                  return
                } else {
                  doReset("e", true)
    ...
    hotkeys: [
        ...
        getCurrentGain() {
          let gain = tmp[this.layer].buyables[11].effect
          return gain
        },
        getCurrentDim1Gain() {
          let gain = tmp[this.layer].buyables[12].effect
          return gain
        },
        getCurrentDim2Gain() {
          let gain = tmp[this.layer].buyables[13].effect
          return gain
        },
        getCurrentDim3Gain() {
          let gain = tmp[this.layer].buyables[14].effect
          return gain
        },
        update(diff) {
          player[this.layer].moreDim1 = player[this.layer].moreDim1.add(layers.e.getCurrentDim1Gain().times(diff))
          player[this.layer].moreDim2 = player[this.layer].moreDim2.add(layers.e.getCurrentDim2Gain().times(diff))
          player[this.layer].moreDim3 = player[this.layer].moreDim3.add(layers.e.getCurrentDim3Gain().times(diff))
          player[this.layer].matter = player[this.layer].matter.add(layers.e.getCurrentGain().times(diff))
        },
    microtabs: {
        main: {
            "Main": {
